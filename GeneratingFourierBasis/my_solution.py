...
import pennylane as qml
    ...
    #1.ans: 0.0,0.0,0.33901,-0.33901,0.0,0.0,0.62054,-0.62054
    #2.ans: -0.28526,-0.18317,0.0,-0.0,-0.33528,-0.33528,-0.0,0.0,-0.18317,-0.28526,-0.0,0.0,-0.52217,-0.52217,-0.0,0.0
...
def generating_fourier_state(n_qubits, m):
    ...
    def error(angles):
        ...
        #print("Probabilities of circuit(angles) are: ", probs)
        
        
        def square_loss(X, Y):                          #square loss to compute the cost
            loss = 0
            #print(list(zip(X,Y)))
            for x, y in zip(X, Y):                      #X=W(theta), Y=[p,1-p]
                loss = loss + (x - y) ** 2
                #print(x, y)
                #len(X)=2 (2 possible outcomes in W(theta))
            return loss / len(X)                        #square_loss=cost           
        
        def cost(angles, p):
            #  p is prob. of ket zero, 1-p is prob of ket 1
            return square_loss(circuit(angles), [p, 1-p])
        #index=np.argmax(probs)
        #error=probs[index]
        #error=float(error)
        #floats = [float(x) for x in probs]
        #print(floats)
        #print(type(error))
    ...
    for epoch in range(epochs):
        angles = opt.step(error, angles) #They are [0,0]
        #Update trainable arguments with one step of the optimizer.
        #print("angles are: ", angles)
        angles = np.clip(opt.step(error, angles), -2 * np.pi, 2 * np.pi) #They still are [0,0]
        #np.clip:  values smaller than -2pi become -2pi, and values larger than 2pi become 2pi.
...
if __name__ == "__main__":
    ...
    print("n_qubits are: ", n_qubits)
    print("m (state we want to generate FT) is: ", m)
